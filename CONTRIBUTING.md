# Contributing to Orion.Abstractions

Thanks for taking the time to look at this. Shared foundation primitives for the Orion family of .NET libraries: fault-safe observer invocation, OpenTelemetry instrumentation conventions (ActivitySource + Meter + static tag stamping), and a testable clock abstraction. Depend on this from any library that wants the Orion conventions; it has no Orion dependencies of its own. The project is small and the bar for contributions is "does it make the package clearer, faster, or safer without expanding the public surface needlessly."
...
```bash
git clone https://github.com/tunahanaliozturk/Orion.Abstractions
cd Orion.Abstractions
dotnet restore
