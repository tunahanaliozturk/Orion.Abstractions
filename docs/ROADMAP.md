...
- **0.2.0** - Instance-scoped instrumentation for `OrionInstrumentation`. An opt-in scoped constructor names the `Meter` via `MeterOptions` carrying a stable `orion.instance` tag (`InstanceTagKey`) plus any custom instance tags and a scope object, so a name-filtered `MeterListener` can no longer confuse two live instances that share a Meter name. Adds the `InstanceScopeId` property and the static `ListensTo(Instrument, OrionInstrumentation)` predicate, which filters a listener to exactly one instance's instruments by Meter reference identity. The parameterless name/version constructor is unchanged and remains the default (no instance tag, no scope).
