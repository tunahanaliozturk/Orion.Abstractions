...
Check(OrionDeadline.After(clock, TimeSpan.Zero).IsExpired, "zero-budget deadline should be expired");
Check(!OrionDeadline.After(clock, TimeSpan.FromHours(1)).IsExpired, "one-hour deadline should not be expired");
Check(!OrionDeadline.Never.IsExpired, "Never should not expire");
